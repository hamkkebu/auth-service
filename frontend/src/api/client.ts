...
apiClient.interceptors.response.use(
  ...
  async (error: AxiosError<ApiResponse<any>>) => {
    ...
    if (error.response) {
      ...
      // 로그인 API는 컴포넌트에서 직접 에러를 처리하므로 인터셉터에서 alert 띄우지 않음
      const isLoginEndpoint = originalRequest.url?.includes('/auth/login');

      // API 응답 에러
      if (data?.error && !isLoginEndpoint) {
      ...
      // 로그인 에러는 로그만 남기고 컴포넌트에서 처리하도록 함
      if (isLoginEndpoint) {
        console.error(`[Login Error ${status}]`, data?.error?.message || data?.message);
      }
